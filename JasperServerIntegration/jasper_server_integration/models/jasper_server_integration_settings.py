...
class JasperServerIntegrationSettings(models.TransientModel):
    ...
    jasper_server_address = fields.Char(
        string="Address",
        help="The address of the Jasper Server. The complete address must follow this pattern: http://<host>:<port>/jasperserver[-pro]/",
    ...
    jasper_server_timeout = fields.Integer(
        string="Timeout",
        help="Timeout for the response of the request. If the response from the server surpasses this value, it will throw an error message.",
        default=lambda self: self._default_timeout()
    )

    def _default_address(self):
        saved_address = self.env['ir.config_parameter'].get_param("jasper_server_integration.address")
        return saved_address if saved_address else ""

    def _default_username(self):
        saved_username = self.env['ir.config_parameter'].get_param("jasper_server_integration.username")
        return saved_username if saved_username else "jasperadmin"

    def _default_password(self):
        saved_password = self.env['ir.config_parameter'].get_param("jasper_server_integration.password")
        return saved_password if saved_password else "jasperadmin"

    def _default_timeout(self):
        saved_timeout = self.env['ir.config_parameter'].get_param("jasper_server_integration.timeout")
    ...
    def validate_server_address(self):
        ...
        if not valid_address_regex.match(self.jasper_server_address):
            raise ValidationError(_("The address used does not follow this pattern: http://<host>:<port>/jasperserver[-pro]/"))
